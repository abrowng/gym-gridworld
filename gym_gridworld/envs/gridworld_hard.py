...
class GridWorldHardEnv(gym.Env):
    ...
    def __init__(self, noise=0.2, terminal_reward=10, start_state=(0,0), width=10, height=10, numWalls=16, rendering=True):
        self.height = height
        self.width = width
        self.noise = noise
        ...
        for i in range(numWalls):
            ...
            while(self.walls[i]==self.start_state or self.walls[i]==self.terminal_state or self.walls[i]==self.loosing_state):
                self.walls[i] = (random.randint(0,self.height-1), random.randint(0, self.width-1))
            self.grid[self.height-self.walls[i][1]-1][self.walls[i][0]] = 3
